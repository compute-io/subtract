...
*	Computes an element-wise subtraction of a typed array.
*
...
function subtract( out, arr, y ) {
	...
	if ( isTypedArrayLike( y ) ) {
		if ( len !== y.length ) {
			throw new Error( 'subtract()::invalid input argument. Array to be multiplied must have a length equal to that of the input array.' );
		}
		for ( i = 0; i < len; i++ ) {
				out[ i ] = arr[ i ] - y[ i ];
		}
